...
getDevInfoById = function (req, res, next) { // 根据id查找device
  ...
  const sql = `SELECT * FROM deviceinf where deviceId = ${deviceId}`
  db.query(sql, [], function (result, fields) {
    if(result!='')
    res.send(resObj(200,result, 'success'));
    else{
      res.send(resObj(200,result, '未查到该设备'));
    }
  })
}

// //删除设备的处理函数
delDevInfoById = (req,res,next)=> { // 根据id删除user
...
//增加设备
 addDevice = function(req,res,next){ // 添加
  let {deviceId,net,deviceSta,appName1,appName2,appName3,appName4,appName5} = req.body
  console.log(req.body)
  const sql = `insert into deviceinf(deviceId,net,deviceSta,appName1,appName2,appName3,appName4,appName5)values( ${deviceId},'${net}',${deviceSta},'${appName1}','${appName2}','${appName3}','${appName4}','${appName5}')`
  db.query(sql, function (result, fields) {
    if (fields !== undefined && result === null) {
      res.send(resObj(200,{deviceId,net,deviceSta,appName1,appName2,appName3,appName4,appName5}, '增加成功'));
    } else {
...
updateDevice = (req,res,next)=>{ // 根据id修改
  ...
  db.query(sql1, [], function (result, fields) {
    ...
    if (result !== []) {
      ...
      const sql2 = `update deviceinf set net = '${net}', deviceSta =${deviceSta} ,appName1 ='${appName1}',appName2 ='${appName2}',appName3 ='${appName3}',appName4 ='${appName4}',appName5 ='${appName5}'where deviceId = ${deviceId}` 
      db.query(sql2, [], function(result2, fields2){
        if (result2 !== null) {
